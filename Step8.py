...
class Person():
    ...
    def __init__(self, name):
        ...
        self.gun = None
        self.charger = None
        self.bullet = []  # bullet 是一个队列，所有的子弹都是放在这个队列的

    '''
        def get_gun(self, gun):  # 人拿枪
            self.gun = gun
        def get_charger(self, charger):  # 人拿弹夹
            self.charger = charger
        def get_bullet(self, bullet):  # 人拿子弹
            self.bullet = bullet
    '''
...
class Gun():
    ...
    def unsave_charger(self):  # 枪可以卸下弹夹
        self.charger = None
...
class Charger():
    ...
    def unsave_bullet(self, person):  # 弹出子弹， 子弹打完了就没有子弹了。弹出一次子弹 就让子弹调用一次射击方法，
        ...
        if len(self.bullet_list) == 0:
            print("弹夹中已经没有了子弹，请更换弹夹")
        else:
            print("弹夹中有%d颗子弹"%len(self.bullet_list))
            self.bullet_list[len(self.bullet_list) - 1].shoot(person)
            # 将最后一颗子弹射出去，并且造成杀伤力
            # 然后弹夹中的子弹减少一颗。
            self.bullet_list.pop(0)  # pop 是干掉队首的bullet对象。
...
def Init():
    ...
    for i in range(12):  # 生成 20颗子弹并安 由李刚安装到 弹夹里面。
        bullet = Bullet(i)  #
        bullet.name = i  # 给子弹进行编号，可以不要
        policeman.install_bullet(bullet, charger)  # 这里需要判断 警察是否有子弹。
        # 安装好了子弹以后，弹夹里面的子弹应该由20课。
        # 警察有射击的功能，当警察射击的时候，抢劫犯就会掉血。 射击的目标是抢劫犯，所以把抢劫犯作为参数传进来。
    policeman.install_charger(charger, gun)  # 安装弹夹 ， 这个需要设计为先有枪，和弹夹才能够安装 弹夹。
    # for j in range(14):
    #    policeman.trigger(gun, robber)  # 警察开始射击
    #    print("警察用%s射中了抢劫犯，抢劫犯的血量变为%d " % (gun.name, robber.blood_count))

    while robber.blood_count > 0:
        if len(charger.bullet_list) > 0:
            policeman.trigger(gun, robber)
            print("警察用%s射中了抢劫犯，抢劫犯的血量变为%d " % (gun.name, robber.blood_count))
        else:
            break
...
if __name__ ==  '__main__':
    ...
    # policeman.gun.unsave_charger() # 卸掉了弹夹。
